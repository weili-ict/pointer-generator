...
def attention_decoder(decoder_inputs, initial_state, encoder_states, cell, initial_state_attention=False, pointer_gen=True, use_coverage=False, prev_coverage=None):
    ...
    Returns:
        outputs: A list of the same length as decoder_inputs of 2D Tensors of shape [batch_size x cell.output_size]. The output vectors.
        state: The final state of the decoder. A tensor shape [batch_size x cell.state_size].
        attn_dists: A list containing tensors of shape (batch_size,attn_length). The attention distributions for each decoder step.
    ...
    with variable_scope.variable_scope("attention_decoder") as scope:
        ...
        def attention(decoder_state, coverage=None):
            ...
            Args:
                decoder_state: state of the decoder [batch_size, attention_vec_size]
...
def linear(args, output_size, bias, bias_start=0.0, scope=None):
    ...
    with tf.variable_scope(scope or "Linear"):
        ...
        if len(args) == 1: #there is only one argument
            res = tf.matmul(args[0], matrix)
        else:
            res = tf.matmul(tf.concat(axis=1, values=args), matrix) #concatenate the args in dimension 1
